...
(* TODO Direction *)
Definition group_effect' (g: groupaction) (s: MatchState): Result MatchState tree_res'_error :=
  match g with
  | Open gid =>
      let cap := MatchState.captures s in
      set cap[gid] := capture_range (MatchState.endIndex s) (-1)%Z in
      Success (match_state (MatchState.input s) (MatchState.endIndex s) cap)
  | Close gid =>
      let cap := MatchState.captures s in
      let! range =<< cap[gid] in
      match range with
      | None => Error MismatchingTree
      | Some (capture_range x y) =>
        set cap[gid] := capture_range x (MatchState.endIndex s) in
        Success (match_state (MatchState.input s) (MatchState.endIndex s) cap)
      end
  | Reset gidl =>
      reset_groups_ms gidl s
  end.

(* returning the highest-priority result *)
(* we also return the corresponding state *)
(* TODO Direction *)
Fixpoint tree_res' (t:tree) (s: MatchState): Result (option MatchState) tree_res'_error :=
  match t with
  | Mismatch => Success None
  | Match => Success (Some s)
  | Choice t1 t2 =>
      let! res1 =<< tree_res' t1 s in
      match res1 with
      | None => tree_res' t2 s
      | Some _ => Success res1
      end
  | Read c t1 => 
    if List.nth (Z.to_nat (MatchState.endIndex s)) (MatchState.input s) c == c then
      tree_res' t1 (advance_ms s)
    else
      Error MismatchingTree
  | CheckFail _ => Success None
  | CheckPass _ t1 => tree_res' t1 s
  | GroupAction g t1 => 
    let! s' =<< group_effect' g s in tree_res' t1 s'
  end.


Definition equiv_tree (mc: MatcherContinuation) (tmc: TMatcherContinuation) :=
  forall s: MatchState, 
  match tmc s, mc s with
  | Error _, _ | _, Error _ => True (* finer modeling? *)
  | Success t, Success res => tree_res' t s = Success res
  end.
...
Lemma id_equiv: equiv_tree id_mcont id_tmcont.
Proof.
...
Theorem compile_tcompile: forall reg ctx rer,
  ...
  forall mc tmc, equiv_tree mc tmc -> equiv_tree (fun s => m s mc) (fun s => tm s tmc)
  end.
Proof.
  intros reg ctx rer.
  revert ctx.
  ...
  - (* Disjunction *)
    ...
    intros.
    intro s'.
    specialize (IHreg1 mc tmc H s').
    specialize (IHreg2 mc tmc H s').
    simpl in *.
  ...
  - (* Sequence *)
    ...
    simpl in *.
    intros. intro.
    specialize (IHreg2 mc tmc H).
    specialize (IHreg1 (fun s0 => m2 s0 mc) (fun s0 => tm2 s0 tmc) IHreg2 s).
    simpl in *.
    assumption.
  - (* Group *)
    intro.
