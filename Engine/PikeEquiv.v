  ...
  - (* when the choice comes from a star *)
    ...
    +                           (* greedy star *)
      ...
      * apply tt_begin; auto.
        ...
        apply tt_reset; auto.
        apply tt_eq with (actions:=Areg r1 :: Acheck(current_str inp forward)::Areg(Quantified true 0 +∞ r1)::cont); auto.
        2: { pike_subset. }
    ...
    +                           (* lazy star *)
      ...
      * constructor. constructor. apply tt_begin; auto.
        replace (S (S pc)) with (S pc + 1) in RESET by lia.
        apply tt_reset; auto.
        apply tt_eq with (actions:=Areg r1 :: Acheck(current_str inp forward)::Areg(Quantified false 0 +∞ r1)::cont); auto.
        2: { pike_subset. }
        apply cons_bc with (pcmid:=end1).
        { constructor. replace (S pc+1+1) with (S (S (S pc))) by lia. auto. }
        repeat (econstructor; eauto).
